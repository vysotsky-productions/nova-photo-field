<template>
  <div class="flex border-b border-40 media-field">
    ...
    <cropper
      v-if="showCropper && original && useCropper"
      :img-src="original"
      :crop-data="originalCropData || {}"
      :extension="extension"
...
export default {
  ...
  methods: {
    ...
    fill(formData) {
      formData.append(
        `${this.field.attribute}_crop_data`,
        this.requestData || ""
      );
  ...
  computed: {
    requestData() {
      return this.cropData ? JSON.stringify(this.cropData) : "";
    },
  },
  mounted() {
    ...
    if (value) {
      ...
      this.originalCropData = value[cropData];
    }
  },
};
</script>
